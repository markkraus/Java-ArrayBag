...
public final class ResizableArrayBag<T> implements BagInterface<T>
{
   ...
   {
		BagInterface<T> resultBag = new ResizableArrayBag<T>();
		ResizableArrayBag<T> argBag = (ResizableArrayBag<T>) anotherBag;
		for(int i = 0; i < numberOfEntries; i++){
			resultBag.add(bag[i]);
		}
		for(int i = 0; i < argBag.numberOfEntries; i++){
			resultBag.add(argBag.bag[i]);
		}
		return resultBag;
	}
   ...
   {
	   BagInterface<T> combinedBag = new ResizableArrayBag<T>();
	   ResizableArrayBag<T> argBag = (ResizableArrayBag<T>) anotherBag;

	   ResizableArrayBag<T> copyBag = new ResizableArrayBag<T>();

		  for(int i = 0; i < argBag.numberOfEntries; i++){
			  copyBag.add(argBag.bag[i]);
		  }

		  for(int i = 0; i < numberOfEntries; i++){
			  T current = bag[i];
			  if(copyBag.remove(current)){
				  combinedBag.add(current);
			  }
		  }
   ...
   public BagInterface<T> difference(BagInterface<T> anotherBag)
   {
	   ...
	   //copy bag into destructible bag
	   ResizableArrayBag<T> copyBag = new ResizableArrayBag<T>();
		for(int i = 0; i < argBag.numberOfEntries; i++){
			copyBag.add(argBag.bag[i]);
		}
		...
		for(int i = 0; i < numberOfEntries; i++){
			T currentValue = bag[i];
			if(!copyBag.remove(currentValue)){
				diffBag.add(currentValue);
			}
		}
